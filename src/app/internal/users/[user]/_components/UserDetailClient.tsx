...
export function UserDetailClient({ userId, initialUser, initialGroups }: UserDetailClientProps) {
  ...
  const [userGroups, setUserGroups] = useState<Group[]>(initialGroups ?? []);
  const [groupsLoading, setGroupsLoading] = useState(initialGroups === null);
  const [rolesModalOpen, setRolesModalOpen] = useState(false);
  const [moderationVisible, setModerationVisible] = useState(true);
  const hydrated = useRef({
    user: initialUser ? userId : null,
    groups: initialGroups ? userId : null,
  });
  ...
  useEffect(() => {
    ...
    if (hydrated.current.user === userId) hydrated.current.user = null;
    else void loadUser();
    if (hydrated.current.groups === userId) hydrated.current.groups = null;
