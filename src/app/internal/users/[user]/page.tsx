  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="flex gap-4  flex-row px-5 py-4">
        <Avatar variant="circle" size={"xl"} name={user.name} src={user.profilePic} />
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center gap-2 text-ink-3">
            ...
            <CopyButton value={user.email} aria-label="Copy email" />
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 flex flex-col gap-4">
          <AccountDetailsCard user={user} lastActive={lastActive} />
          <UserGroupsSection groups={MOCK_GROUPS} />
        </div>
        <div>
          <UserModerationCard user={user} />
        </div>
      </div>
    </div>
  );
