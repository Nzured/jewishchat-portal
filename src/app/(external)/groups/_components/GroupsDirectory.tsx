...
export interface GroupsDirectoryProps {
  initialKey: string;
  initialResults: GroupSearchResults | null;
  categories: Category[];
}

export function GroupsDirectory({ initialKey, initialResults, categories }: GroupsDirectoryProps) {
  ...
  const [totalResults, setTotalResults] = React.useState(() => initialResults?.totalResults ?? 0);
  const [searchId, setSearchId] = React.useState<string | undefined>(initialResults?.searchId);
  const [isLoading, setIsLoading] = React.useState(false);
  const [isLoadingMore, setIsLoadingMore] = React.useState(false);
  const [autoCompleteResults, setAutoCompleteResults] = React.useState<SearchSuggestion[]>([]);
  const loadedFilters = React.useRef<string | null>(initialKey);
  ...
  const debouncedPrefix = useDebouncedValue(value.trim(), HERO_SEARCH_DEBOUNCE_MS);

  React.useEffect(() => {
    if (debouncedPrefix.length < SEARCH_MIN_LENGTH) return;

    let ignore = false;

    SearchService.autoComplete(debouncedPrefix)
      .then((res) => {
  ...
  React.useEffect(() => {
    const key = searchKey({ q: query, category, city, country, sort });
  ...
  const updateSearchParams = (updates: Record<string, string | undefined>) => {
    ...
    window.history.replaceState(null, "", `${EXTERNAL_GROUPS_PATH}${qs ? `?${qs}` : ""}`);
  };
