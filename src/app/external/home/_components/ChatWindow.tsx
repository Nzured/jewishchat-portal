...
export function ChatWindow() {
  ...
  const settledQuery = isSearching ? null : query;
  ...
  useEffect(() => {
    ...
    const buildReply = (): ChatMessage => ({
      ...
      href: `${EXTERNAL_GROUPS_PATH}?q=${encodeURIComponent(trimmed)}`,
      time: nowLabel(),
    });
    ...
    const holdMs = 600 + Math.round(Math.random() * 300);
    const timer = window.setTimeout(() => {
      setMessages((prev) => prev.filter((m) => m.id !== typingMsg.id).concat(buildReply()));
    }, holdMs);
